...
관리 화면은 언어 설정을 행 단위 인라인 그리드로, 번역을 `메시지 키 × 등록 언어` 매트릭스로 구성합니다. 번역 행 하나를 저장할 때 여러 언어 값을 한 트랜잭션에서 검증·반영하며, 번들 값과 같은 값이나 빈 셀은 DB 오버라이드를 제거하는 복원 동작으로 처리합니다. 동적 번역은 모든 화면에서 일반 텍스트로 사용되므로 저장 전에 HTML 괄호와 제어 문자를 거부하고 `MessageFormat` 치환식도 검증합니다.

기본 메뉴는 메뉴 키를 메시지 키로 변환해 번역하고, 메시지가 없는 사용자 정의 메뉴는 DB에 저장된 이름과 그룹을 그대로 사용합니다. 탭 복원 데이터에는 메뉴 키만 남기므로 언어를 바꾼 뒤 셸을 다시 읽으면 열린 탭도 새 메뉴 이름으로 재구성됩니다.
