...
| 변수 | 기본값 | 운영 메모 |
|---|---:|---|
| `APP_I18N_DEFAULT_LOCALE` | `en` | DB 언어 카탈로그가 비어 있을 때 사용하는 BCP 47 대체값 |
| `APP_I18N_SUPPORTED_LOCALES` | `en,ko,zh,ja` | DB 언어 카탈로그가 비어 있을 때 사용하는 대체 목록 |
...
| `APP_I18N_CACHE_MAX_ENTRIES` | `50000` | 동적 번역 조회 캐시의 최대 키 수 |
| `APP_I18N_CACHE_TTL` | `5m` | 애플리케이션 밖에서 DB를 바꾼 경우 다시 조회할 최대 대기 시간 |

언어 쿠키는 자바스크립트에서 읽을 필요가 없으므로 `HttpOnly`이며, 교차 사이트 요청에 불필요하게 실리지 않도록 `SameSite=Lax`를 사용합니다. 언어 변경 엔드포인트는 DB에서 활성화된 정확한 언어 태그만 저장하고, 직접 변조된 쿠키 값도 해석 단계에서 활성 목록으로 다시 제한합니다. 화면 복귀 주소도 같은 애플리케이션의 정규화된 절대 경로만 허용하므로, 프록시나 애플리케이션이 이 검증을 우회해 외부 URL을 주입하지 않게 유지하세요.

관리자는 다국어 관리 그리드의 각 행에서 BCP 47 태그, 관리용 이름, 언어 고유 표시명, 순서와 활성 상태를 직접 수정하고 기본 언어를 바꿀 수 있습니다. 번역 화면은 메시지 키를 행, 등록 언어를 열로 표시하므로 한 행에서 여러 언어를 비교·수정한 뒤 한 번만 저장합니다. 셀을 비우거나 번들 값과 같게 저장하면 DB 오버라이드가 제거됩니다. 트랜잭션 커밋 뒤 캐시가 비워져 다음 요청부터 적용되며, 애플리케이션 외부에서 DB를 직접 수정하면 캐시 TTL까지 기존 값이 보일 수 있으므로 운영 변경은 관리 화면을 기준으로 삼으세요.

영어·한국어·중국어·일본어 번들은 같은 키 집합을 가지며 자동 테스트가 누락과 빈 값을 검사합니다. 새 언어를 운영에 추가할 때 DB 등록만으로 언어 선택과 DB 번역은 사용할 수 있지만, 오류·검증 문구를 포함한 완전한 기준 번들이 필요하면 `messages_<언어태그>.properties`를 추가하고 일관성 테스트 대상도 함께 확장하세요. 이메일이나 내보내기처럼 별도 출력 채널의 번역은 이 화면과 별도로 검토해야 합니다.
