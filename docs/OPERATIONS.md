...
관리자는 다국어 관리 화면에서 BCP 47 태그, 관리용 이름, 언어 고유 표시명, 순서와 활성 상태를 등록하고 기본 언어를 바꿀 수 있습니다. 번역 편집 화면에 저장한 문구는 트랜잭션 커밋 뒤 캐시가 비워져 다음 요청부터 적용됩니다. 번들 값으로 되돌리려면 DB 오버라이드를 삭제합니다. 애플리케이션 외부에서 DB를 직접 수정하면 캐시 TTL까지 기존 값이 보일 수 있으므로 운영 변경은 관리 화면을 기준으로 삼으세요.
