...
- 管理员可在界面中注册、修改、排序、启用语言，并选择默认语言
- 可搜索任意消息包键，并将 MariaDB 中的翻译覆盖值保存到该语言
...
要在运行时添加语言，请在 **多语言管理** 中登记 BCP 47 标签并录入翻译。若新语言需要随应用提供完整、审核后的默认文本，可再添加 `messages_<语言标签>.properties`。自动测试会检查四个内置消息包的键和值是否一致且非空。
...
| 语言目录 | 翻译编辑器 |
|---|---|
| ![运行时语言目录](docs/images/localization.png) | ![数据库翻译编辑器](docs/images/translations.png) |
