...
- 언어 그리드의 각 행에서 BCP 47 태그·이름·순서·사용 여부를 직접 수정하고 행 단위로 저장
- 메시지 키를 행, 등록 언어를 열로 배치한 번역 매트릭스에서 여러 언어를 동시에 비교·편집
- 번역 셀을 비우면 번들 문구로 복원되며, MariaDB 변경은 커밋 다음 요청부터 즉시 적용
...
새 언어는 **다국어 관리**의 신규 행에 BCP 47 태그를 입력한 뒤 번역 매트릭스에 생긴 열을 채우면 바로 사용할 수 있습니다. 한 메시지 행에서 여러 언어를 수정하고 한 번만 저장하며, 번역 셀에서는 `Ctrl/⌘ + Enter`로도 저장할 수 있습니다. 완전히 검토된 기본 번들을 함께 배포하려면 `messages_<언어태그>.properties`를 추가합니다. `MessageBundleConsistencyTest`는 기본 네 언어의 모든 키와 동적 열거형 메시지가 빠지지 않았는지 확인합니다.
...
| 인라인 언어 그리드 | 다국어 번역 매트릭스 |
|---|---|
| ![인라인 동적 언어 그리드](docs/images/localization.png) | ![DB 다국어 번역 매트릭스](docs/images/translations.png) |
...
| 다국어 관리 | 언어 행과 메시지×언어 번역 매트릭스 직접 편집 | 관리자 |
