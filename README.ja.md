...
- 管理画面から BCP 47 言語を登録・編集・並べ替え・有効化し、既定言語を変更
- バンドルのメッセージキーを検索し、MariaDB の翻訳値で上書き
...
実行中に言語を追加するには、**多言語管理**で BCP 47 タグと翻訳を登録します。完全にレビューした初期翻訳をアプリケーションと一緒に配布する場合は、`messages_<言語タグ>.properties` も追加してください。自動テストは内蔵 4 言語のキー集合、空値、動的列挙メッセージを検証します。
...
| 言語カタログ | 翻訳エディター |
|---|---|
| ![実行時言語カタログ](docs/images/localization.png) | ![DB 翻訳エディター](docs/images/translations.png) |
...
| 多言語管理 | 言語登録、既定言語選択、DB 翻訳の編集 | 管理者 |
